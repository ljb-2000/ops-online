...
def gen_ssh_key(username, password=None, length=2048):
    private_key_dir = os.path.join(BASE_DIR, 'keys/ldapserver/')
...
def dept_add(request):
    ...
    return render_to_response('dept_add.html', locals(), context_instance=RequestContext(request))
...
def dept_list(request):
    ...
    return render_to_response('dept_list.html', locals(), context_instance=RequestContext(request))
...
def dept_detail(request):
    ...
    if not dept_id:
        return HttpResponseRedirect('/ldapuser/dept_list/')
    dept = DEPT.objects.filter(id=dept_id)   
    if dept:
        dept = dept[0]
        users = dept.user_set.all()    
    return render_to_response('dept_detail.html', locals(), context_instance=RequestContext(request))
...
def dept_edit(request):
    ...
    if request.method == 'GET':
        ...
        if dept:
            ...
            error = '部门不存在'
        return HttpResponseRedirect('/ldapuser/dept_list/')
    return render_to_response('dept_edit.html', locals(), context_instance=RequestContext(request))
...
def dept_user_ajax(request):
    ...
    return render_to_response('dept_user_ajax.html', locals())
...
def group_add(request):
    ...
    return render_to_response('group_add.html', locals(), context_instance=RequestContext(request))
...
def group_list(request):
    ...
    contact_list, p, contacts, page_range, current_page, show_first, show_end = pages(contact_list, request)
    return render_to_response('group_list.html', locals(), context_instance=RequestContext(request))
...
def group_detail(request):
    ...
    return render_to_response('group_detail.html', locals(), context_instance=RequestContext(request))
...
def group_del(request):
    ...
    return HttpResponseRedirect('/ldapuser/group_list/')
...
def group_edit(request):
    ...
    if request.method == 'GET':
        ...
        return render_to_response('group_edit.html', locals(), context_instance=RequestContext(request))
    else:
    ...
    if request.method == 'POST':
		...
		mail_title = u'恭喜你的跳板机用户添加成功'
                mail_msg = """
                    ...
                    您的web登录密码： %s
                    您的ssh密钥文件密码： %s
                    说明：密钥在附件中！(登陆 124.251.36.59 使用的SSH密钥)
			   web密码为登录运维管理平台(ops.tclclouds.com)的密码！
                """ % (name, username, dept.name, user_role.get(role_post, ''),
                       password, ssh_key_pwd)
		...
		private_key_dir = os.path.join(BASE_DIR, 'keys/ldapserver/')
		private_key_file = os.path.join(private_key_dir, username+".pem")
		mail=EmailMessage(mail_title,mail_msg,MAIL_FROM,[email])
		mail.attach_file(private_key_file)
...
def user_list_adm(request):
    ...
    if gid:
        if not validate(request, user_group=[gid]):
            return HttpResponseRedirect('/ldapuser/user_list/')
        user_group = UserGroup.objects.filter(id=gid)
        if user_group:
            user_group = user_group[0]
            contact_list = user_group.user_set.all()
...
def user_edit_adm(request):
    ...
    if request.method == 'GET':
        ...
        if not validate(request, user=[user_id], user_group=groups):
            return HttpResponseRedirect('/ldapuser/user_edit/')
        if user_id:
            user = User.objects.filter(id=user_id)
            if user:
                user = user[0]
...
def chg_info(request):
    ...
    return render_to_response('chg_info.html', locals(), context_instance=RequestContext(request))
